...
[...Array(npages).keys()].forEach( page => {
	...
	// // layer
	( () => {
		let colors = [[pigments.black,2], [pigments.white,5], [pigments.blue,0], [pigments.yellow,0], [pigments.red,0]].reduce( (acc, color, j) => {
	...
	// layer
	( () => {
		let colors = [[pigments.black,4], [pigments.white,8], [pigments.blue,0], [pigments.yellow,0], [pigments.red,0]].reduce( (acc, color, j) => {
			Array.prototype.push.apply(acc,[...Array(color[1]).keys()].reduce( (acc2,k) => { acc2.push(color[0]); return acc2 },[]) );
			return acc;
		}, []);
		let colorj=0, color = colors[0];
	...
	// layer
	( () => {
		let colors = [[pigments.black,2], [pigments.white,3], [pigments.blue,0], [pigments.yellow,0], [pigments.red,0]].reduce( (acc, color, j) => {
			Array.prototype.push.apply(acc,[...Array(color[1]).keys()].reduce( (acc2,k) => { acc2.push(color[0]); return acc2 },[]) );
			return acc;
		}, []);
		let colorj=0, color = colors[0];
