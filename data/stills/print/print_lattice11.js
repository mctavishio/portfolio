...
[...Array(npages).keys()].forEach( page => {
	...
	( () => {
		...
		[...Array(4).keys()].forEach( (d,j) => {
			[0,2.5,5,7.5,10].map(d => Math.floor(width*d/10)).map( cx => {
	...
	( () => {
		...
		[...Array(4).keys()].forEach( (d,j) => {
			[0,3,7,10].map(d => Math.floor(height*d/10)).map( cy => {
	...
	( () => {
		...
		let r=()=>tools.randominteger(8,dx*.4), dash=()=>tools.randominteger(10, 200);
		[...Array(4).keys()].forEach( (d,j) => {
			[5,7.5,10].map(d => Math.floor(width*d/10)).map( cx => {
				[10,8,6,4,2,1].map(d => Math.floor(dx*d/10)).map( lw => {
				colorj = tools.randominteger(0,colors.length); color = colors[colorj];
				doc.moveTo(cx,y1)
					.lineTo(cx,y2)
	...
	( () => {
		...
		let r=()=>tools.randominteger(8,dy*.4), dash=()=>tools.randominteger(10, 100);
		[...Array(4).keys()].forEach( (d,j) => {
			[0,3].map(d => Math.floor(height*d/10)).map( cy => {
				[10,8,6,4,2,1].map(d => Math.floor(dy*d/10)).map( lw => {
				colorj = tools.randominteger(0,colors.length); color = colors[colorj];
				doc.moveTo(x1,cy)
					.lineTo(x2,cy)
	...
	doc.rect(0, 0, width, height).strokeColor(pigments.black).dash(tools.randominteger(width/2,width)).lineWidth(height*.04).stroke();
	...
	( () => {
		...
		let r=()=>tools.randominteger(8,dx*.4), dash=()=>tools.randominteger(10, 200);
		[...Array(4).keys()].forEach( (d,j) => {
			[5].map(d => Math.floor(width*d/10)).map( cx => {
				[4,2,1].map(d => Math.floor(dx*d/10)).map( lw => {
				colorj = tools.randominteger(0,colors.length); color = colors[colorj];
				doc.moveTo(cx,y1)
					.lineTo(cx,y2)
					.dash(dash(), {space: dash()})
