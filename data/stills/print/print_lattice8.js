...
[...Array(npages).keys()].forEach( page => {
	...
	// // layer
	( () => {
		let colors = [[pigments.black,4], [pigments.white,10], [pigments.blue,0], [pigments.yellow,0], , [pigments.red,1]].reduce( (acc, color, j) => {
		...
		[...Array(16).keys()].forEach( (d,j) => {
			[9,8,6,4].map(d => Math.floor(r*d/8)).map( d => {
	...
	// // layer
	( () => {
		let colors = [[pigments.black,8], [pigments.white,10], [pigments.blue,0], [pigments.yellow,1], , [pigments.red,1]].reduce( (acc, color, j) => {
			Array.prototype.push.apply(acc,[...Array(color[1]).keys()].reduce( (acc2,k) => { acc2.push(color[0]); return acc2 },[]) );
			return acc;
		}, []);
		let colorj=0, color = colors[0];
	...
	// layer
	( () => {
		let colors = [[pigments.black,2], [pigments.white,2], [pigments.blue,0], [pigments.yellow,0], , [pigments.red,0]].reduce( (acc, color, j) => {
			Array.prototype.push.apply(acc,[...Array(color[1]).keys()].reduce( (acc2,k) => { acc2.push(color[0]); return acc2 },[]) );
			return acc;
		}, []);
		let colorj=0, color = colors[0];
		...
		let cx=width/2,cy=height/2, min=Math.min(width,height), r=Math.floor(min*0.2);
		[...Array(4).keys()].forEach( (d,j) => {
			[10,8,6,4,2].map(d => Math.floor(r*d/10)).map( d => {
			// [9,6,4].map(d => Math.floor(r*d/10)).map( d => {
				colorj = tools.randominteger(0,colors.length); color = colors[colorj];
				doc.moveTo(0,cy)
					.lineTo(width,cy)
	...
	// layer
	( () => {
		let colors = [[pigments.black,0], [pigments.white,5], [pigments.blue,0], [pigments.yellow,0], , [pigments.red,2]].reduce( (acc, color, j) => {
			Array.prototype.push.apply(acc,[...Array(color[1]).keys()].reduce( (acc2,k) => { acc2.push(color[0]); return acc2 },[]) );
			return acc;
		}, []); 
		let colorj=0, color = colors[0];
		// console.log("colors = " + JSON.stringify(colors));
		let min = Math.min(width,height), r=Math.floor(min*0.2/2);
		let grid=[0,2,4,6,8,10]; let cr = r*3/10;
		let coords = [];
		grid.map( d => d/10*width ).forEach( cx => {
			grid.map( d => d/10*height ).forEach( cy => {
				if(tools.randominteger(0,10)<4) {
					[9,6].map(d => Math.floor(r*d/10)).forEach( cr => {
						colorj = tools.randominteger(0,colors.length); color = colors[colorj];
						doc.circle(cx, cy, cr).fillColor(color).fill();

					});
					color = pigments.black;
					doc.circle(cx, cy, cr).fillColor(color).fill();
					coords.push([cx,cy]);
				}
			})
		...
		let lw = cr/4;
		// coords.filter(d=>tools.randominteger(0,10)<3).forEach( (d,j) => {
		// 	let next = coords[(j+1)%coords.length];
		// 	doc.moveTo(d[0],d[1])
		// 			.lineTo(next[0],next[1]).dash(width*2)
		// 			.strokeColor(pigments.red).lineWidth(lw)             
		// 			.stroke();
		// 	// doc.circle(d[0], d[1], lw*2).fillColor(pigments.red).fill();
		// });
		// tools.shuffle(coords).filter(d=>tools.randominteger(0,10)<3).forEach( (d,j) => {
		// 	let next = coords[(j+1)%coords.length];
		// 	doc.moveTo(d[0],d[1])
		// 			.lineTo(next[0],next[1]).dash(width*2)
		// 			.strokeColor(pigments.black).lineWidth(lw)             
		// 			.stroke();
		// 	// doc.circle(d[0], d[1], lw*2).fillColor(pigments.black).fill();
		// });
		// tools.shuffle(coords).filter(d=>tools.randominteger(0,10)<2).forEach( (d,j) => {
		// 	let next = coords[(j+1)%coords.length];
		// 	doc.moveTo(d[0],d[1])
		// 			.lineTo(next[0],next[1]).dash(width*2)
		// 			.strokeColor(pigments.yellow).lineWidth(lw)             
		// 			.stroke();
		// 	// doc.circle(d[0], d[1], lw*2).fillColor(pigments.black).fill();
		// });
		// tools.shuffle(coords).forEach( (d,j) => {
		// 	let next = coords[(j+1)%coords.length];
		// 	doc.moveTo(d[0],d[1])
		// 			.lineTo(next[0],next[1]).dash(width*2)
		// 			.strokeColor(pigments.white).lineWidth(lw)             
		// 			.stroke();
		// 	// doc.circle(d[0], d[1], lw*2).fillColor(pigments.black).fill();
		// });
		...
		coords.forEach( (d,j) => {
			// let next = coords[(j+1)%coords.length];
			doc.circle(d[0], d[1], lw*1.4).fillColor(pigments.red).fill();
			// doc.circle(next[0], next[1], cr*tools.randominteger(2,4)/10).fillColor(color).fill();
		});
