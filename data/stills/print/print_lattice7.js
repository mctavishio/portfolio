...
// const width = (8.5 + 0.125)*72*2.1, height = (8.5 + 0.25)*72;
...
const title = "print_lattice7_8.5x8.5_" + Date.now().toString();
...
[...Array(npages).keys()].forEach( page => {
	...
	// 	let min = Math.min(width,height), r=Math.floor(min*0.4);
	// 	let grid=[0,2,4,6,8,10].map( d => { return {x: width*d/10, y: height*d/10} } );
	// 		grid.map( d => {
	// 			[6,4,2].map(d => Math.floor(r*d/10)).map( d => {
	...
	// 			})
	// 	})
	// 	r=Math.floor(height*0.4);
	// 	let ygrid=[2,4,6,8].map( d=> height*d/10 );
	// 	ygrid.map( cy => {
	// 		[6,4,2].map(d => Math.floor(r*d/10)).map( d => {
	// 		// [9,6,4].map(d => Math.floor(r*d/10)).map( d => {
	// 			colorj = tools.randominteger(0,colors.length); color = colors[colorj];
