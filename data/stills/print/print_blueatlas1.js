...
const title = "print_blueatlas1_" + Date.now().toString();
const info = { doctitle: title, Title: "blue atlas", Author: "mctavish", Subject: "blue atlas", Keywords: "net.art, webs, networks" };
const npages = 300, nshapes = 4, dx = Math.floor(width/nshapes)+1, dy = Math.floor(height/nshapes)+1;
...
[...Array(npages).keys()].forEach( page => {
	...
	[...Array(nshapes).keys()].forEach( s => {
		...
		let r = tools.randominteger(10,height), w = dx, h = height;
		// let x = tools.randominteger(0,width-10), y = height-h;
		...
		if(dofill) {
			...
			doc.circle(cx, cy, r )
				...
				.strokeColor(color).lineWidth(tools.randominteger(4,r/2))
				.stroke()
			color = colors[tools.randominteger(0,colors.length)];
			doc.circle(cx, cy, r )
				// .dash(tools.randominteger(r/2,r*r))
	...
	[...Array(nshapes).keys()].forEach( s => {
		...
		let dofill = (tools.randominteger(0,10) < 0 &&  r < height/2);
		color = colors[tools.randominteger(0,colors.length)];
		if(dofill) {
			doc.circle(cx, cy, r)
				.fillColor(color).fill()
			...
			doc.circle(cx, cy, r )
				.dash(tools.randominteger(r/4,r*r))
				.strokeColor(color).lineWidth(tools.randominteger(4,r/4))
				.stroke()
			color = colors[tools.randominteger(0,colors.length)];
			doc.circle(cx, height, r )
				.dash(tools.randominteger(r/4,r*r))
				.strokeColor(color).lineWidth(tools.randominteger(4,r/4))
				.stroke()
		}
	...
	// 		// let r = tools.randominteger(10,height), w = tools.randominteger(10,width), h = tools.randominteger(10,height);
	// 		let w = dx, h = tools.randominteger(10,height/4);
	// 		// let x = tools.randominteger(0,width-dx), y = height-h;
	// 		let x = s*dx, y = height-h;
	// 		let color = colors[tools.randominteger(0,colors.length)];
	// 		doc.rect(x, y, w, h).fillColor(color).fill();
	// 	}
