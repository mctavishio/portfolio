...
[...Array(npages).keys()].forEach( page => {
	...
	( () => {
		...
		let cx=width/2,cy=height/2, min=Math.min(width,height), r=Math.floor(min*0.4);
		[...Array(16).keys()].forEach( (d,j) => {
			color = pigments.white;
			doc.circle(cx, cy, r)
				.dash(tools.randominteger(r/6,r/4), {space: tools.randominteger(r/4,r/2) })
				.strokeColor(color).lineWidth(tools.randominteger(r/6,r/4))
				.stroke();
			[9,8,6,5,4,3].map(d => Math.floor(r*d/10)).map( d => {
				colorj = tools.randominteger(0,colors.length); color = colors[colorj];
